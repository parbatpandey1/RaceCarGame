...
CarType showCarSelection(RenderWindow& window) {
    Font font;
    if (!font.loadFromFile("fonts/OpenSans.ttf")) {
        if (!font.loadFromFile("Fonts/OpenSans.ttf")) {
            return NORMAL_CAR; // Default to normal car if font not found
        }
    }

    // Load car selection images
    Texture normalCarTex, policeCarTex;
    Sprite normalCarSprite, policeCarSprite;

    bool hasNormalCarImg = normalCarTex.loadFromFile("images/choosecar.png");
    bool hasPoliceCarImg = policeCarTex.loadFromFile("images/choosepolice.png");

    if (hasNormalCarImg) {
        normalCarSprite.setTexture(normalCarTex);
        // Scale and position normal car image
        float scale = 200.0f / normalCarTex.getSize().x; // Scale to 200px width
        normalCarSprite.setScale(scale, scale);
        normalCarSprite.setPosition(WIDTH / 4 - 100, HEIGHT / 2 - 50);
    }

    if (hasPoliceCarImg) {
        policeCarSprite.setTexture(policeCarTex);
        // Scale and position police car image
        float scale = 200.0f / policeCarTex.getSize().x; // Scale to 200px width
        policeCarSprite.setScale(scale, scale);
        policeCarSprite.setPosition(3 * WIDTH / 4 - 100, HEIGHT / 2 - 50);
    }

    Text title("CHOOSE YOUR CAR", font, 50);
    title.setFillColor(Color::Yellow);
    title.setPosition(WIDTH / 2 - title.getGlobalBounds().width / 2, 100);

    vector<Text> carOptions(2);
    carOptions[0].setString("NORMAL CAR");
    carOptions[1].setString("POLICE CAR");

    for (int i = 0; i < 2; i++) {
        carOptions[i].setFont(font);
        carOptions[i].setCharacterSize(30);
        if (i == 0) {
            carOptions[i].setPosition(WIDTH / 4 - carOptions[i].getGlobalBounds().width / 2, HEIGHT / 2 + 120);
        }
        else {
            carOptions[i].setPosition(3 * WIDTH / 4 - carOptions[i].getGlobalBounds().width / 2, HEIGHT / 2 + 120);
        }
        carOptions[i].setFillColor(i == 0 ? Color::Red : Color::White);
    }

    Text instruction("Use Arrow Keys to Select, Enter to Confirm", font, 20);
    instruction.setFillColor(Color::Cyan);
    ...
    while (window.isOpen()) {
        Event e;
        while (window.pollEvent(e)) {
            if (e.type == Event::Closed) return NORMAL_CAR;
            if (e.type == Event::KeyPressed) {
                if (e.key.code == Keyboard::Left || e.key.code == Keyboard::A) {
                    carOptions[selected].setFillColor(Color::White);
                    selected = (selected - 1 + carOptions.size()) % carOptions.size();
                    carOptions[selected].setFillColor(Color::Red);
                }
                else if (e.key.code == Keyboard::Right || e.key.code == Keyboard::D) {
                    carOptions[selected].setFillColor(Color::White);
                    selected = (selected + 1) % carOptions.size();
                    carOptions[selected].setFillColor(Color::Red);
                }
                else if (e.key.code == Keyboard::Enter || e.key.code == Keyboard::Space) {
                    return (selected == 0) ? NORMAL_CAR : POLICE_CAR;
                }
            }
        }
        ...
        // Animate title
        float time = clock.getElapsedTime().asSeconds();
        title.setScale(1.0f + sin(time * 2) * 0.03f, 1.0f + sin(time * 2) * 0.03f);

        window.clear(Color(20, 20, 40));
        window.draw(title);
        window.draw(instruction);

        // Draw car images if loaded
        if (hasNormalCarImg) window.draw(normalCarSprite);
        if (hasPoliceCarImg) window.draw(policeCarSprite);

        // Highlight selected car with a border
        if (selected == 0 && hasNormalCarImg) {
            RectangleShape border(Vector2f(220, 220));
            border.setFillColor(Color::Transparent);
            border.setOutlineColor(Color::Red);
            border.setOutlineThickness(3);
            border.setPosition(WIDTH / 4 - 110, HEIGHT / 2 - 60);
            window.draw(border);
        }
        else if (selected == 1 && hasPoliceCarImg) {
            RectangleShape border(Vector2f(220, 220));
            border.setFillColor(Color::Transparent);
            border.setOutlineColor(Color::Red);
            border.setOutlineThickness(3);
            border.setPosition(3 * WIDTH / 4 - 110, HEIGHT / 2 - 60);
            window.draw(border);
        }

        for (auto& opt : carOptions) window.draw(opt);
        window.display();
    }
    return NORMAL_CAR;
}
...
int main() {
    ...
    if (selectedCar == NORMAL_CAR) {
        ...
        if (!playerCarTex.loadFromFile("images/mainpolice.png")) {
            cerr << "Warning: mainpolice.png not found" << endl;
        }
    }
